...
This repository remains an architecture and bootstrap project. It is not a working Discord client and does not claim tested Discord interoperability. The visible application combines a fabricated in-memory client shell with an experimental remote-auth checkpoint. The live transport can reach and verify `pending_remote_init` and render a real QR payload, but it intentionally stops before account approval and login. Token exchange, a real authenticated account, Gateway and REST sessions, Discord message synchronization, persistence, sending, voice and video, DAVE encryption, screen sharing, notifications, distribution, and release operations remain unimplemented or unverified.
...
The Swift package now exposes four dependency-free libraries. DiscordCore contains lossless Snowflake values, tolerant Gateway opcode values, connection-state data, deterministic exponential backoff, server-provided rate-limit observations, and a credential-attachment policy that permits credentials only on exact Discord API origins and API paths. AuthFeature contains a transport-free remote-auth state machine, an actor coordinator, redacted account models, an in-memory fixture store, and the tested Security.framework-backed `KeychainCredentialStore`. CacheCore contains account namespaces, authorization-versus-presence state, retention bounds, draft and pending-send records, and explicit removal commands. RemoteAuthTransport contains the bounded WebSocket, RSA-OAEP, message-validation, fingerprint-verification, and login-exchange contracts used by the phase-1 checkpoint.

These modules are contracts and safety primitives, not a complete client. The Keychain adapter and remote-auth transport are implemented and covered by automated tests, but the live UI deliberately does not exchange a login ticket, decrypt or persist a Discord token, or start an authenticated Gateway session. The in-memory credential store remains fixture-only. No REST client, message synchronizer, database engine, or media bridge is implemented.

## Architecture

The XcodeGen manifest in `project.yml` is the source of truth for the macOS application project. `Package.swift` provides a reproducible headless build and test path for DiscordCore, AuthFeature, CacheCore, and RemoteAuthTransport. Generated Xcode project files are derived artifacts and should not be hand-edited.

The application target contains the macOS entry point, scene setup, commands, views, and temporary presentation state. The package modules are intentionally independent of SwiftUI and AppKit. `RemoteAuthTransport` is used by the experimental authentication view for the credential-free QR checkpoint; `KeychainCredentialStore` is available as a tested account-scoped credential adapter but is not reached by the incomplete live login path. Future transport, persistence, feature, and media adapters should preserve that dependency direction. Long-running work belongs in actors, while UI state remains main-actor isolated under Swift 6 concurrency checking.
...
Authentication is an early feasibility gate, not an assumption hidden behind the interface. Discord's ordinary documented OAuth2 application flow is not a general replacement-client login mechanism, and a bot-oriented Swift library does not remove those restrictions. The current experimental UI connects to the unofficial remote-auth WebSocket, performs the hello/init/nonce-proof sequence, verifies the public-key fingerprint on `pending_remote_init`, and renders the resulting QR. It intentionally closes when `pending_ticket` or `pending_login` arrives. No token exchange, real login, account import, or authenticated Gateway session works yet.

The first authentication proof must cover approval, rejection, expiry, cancellation, session restoration, logout, invalidation, and account-hint matching. The current automated suite covers the transport and credential contracts, and 68 automated tests pass; no live account smoke test or scan/approval test has run. MFA, CAPTCHA, and any official-page flow must be tested without bypassing the challenge before support is promised. Credentials, QR session secrets, and messages must never pass through project infrastructure. Use a disposable account for any future supervised scan, and never paste a token into chat, an issue, a log, or a coding-agent session.
...
The package libraries can be built without generating the Xcode project by running `swift build` from this directory. Run `swift test` to execute the DiscordCore, AuthFeature, CacheCore, and RemoteAuthTransport test targets; the current suite contains 68 passing automated tests. The generated Xcode scheme also defines the application test target and coverage collection, which require an actual macOS Xcode destination. No Discord account, token, client secret, signing identity, or production service is needed for automated tests.
...
The present suite covers Snowflake wire preservation, Gateway opcode tolerance, deterministic backoff, authentication lifecycle transitions, redacted credential descriptions, Keychain account isolation and deletion behavior through a fake Security.framework client, cache namespace isolation, retention bounds, draft and pending-send invariants, exact Discord API credential origins, redirect re-evaluation, remote-auth RSA-OAEP and base64 fixtures, hello/init/nonce-proof/pending-remote-init transitions, fingerprint rejection, bounded messages, heartbeat failure, login-exchange capability boundaries, and tolerant parsing of route and global rate-limit metadata. The 68 automated tests use fabricated values and protocol fakes; they do not perform a live account smoke test.
